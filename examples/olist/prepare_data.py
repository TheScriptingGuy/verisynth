...
def prepare(csv_dir: Path, out_dir: Path, sample_customers: int) -> dict[str, int]:
    ...
    # ``contact_id`` mirrors ``customer_id`` (both ``customer_unique_id``): in
    # the real Olist data the shop customer *is* the CRM contact, so the two
    # ids coincide here (see prepare_crm_data.py / TASK CARD 12, where a
    # synthetic CRM source links to the shop customer via ``contact_id``, and
    # the two diverge numerically once ids are regenerated by the engine).
    customers_df = (
        ...
        .with_columns(pl.col("customer_id").alias("contact_id"))
    )
